...
scenarios = json.load(open(os.path.join(path_n, "output", "select.JSON")))
...
for col in ncols:
    ...
    for row in nrows:
        ...
        for i, scenario in enumerate(scenarios):
            ...
            sel = [target_col - 1 for target_col in scenarios[scenario][col]]
            print(sel)
            for j in range(RUNS):
                temp = data
                timings.append(time_function(func=f"temp.iloc[:,{sel}]"))
...
results_df.to_csv(os.path.join(path_n, "output", "select_results_pandas.csv"), index=False)